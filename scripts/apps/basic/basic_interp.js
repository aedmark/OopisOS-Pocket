...
window.Basic_interp = class Basic_interp {
  ...
  async executeStatement(statement) {
    ...
    if (!match) {
      if (statement.includes("=")) {
        ...
        throw new Error(
            `Syntax Error: Invalid statement format '${statement}'`
        );
      }
      return;
    }
    ...
    switch (command) {
      ...
      case "LET": {
        ...
        const arrayMatch = varNameStr.match(
            /([a-zA-Z_][a-zA-Z_0-9$]*)\((.*)\)/
        );
        if (arrayMatch) {
          ...
          if (!this.arrays.has(arrayName))
            throw new Error(`Array not dimensioned: ${arrayName}`);
          const arr = this.arrays.get(arrayName);
          if (index < 0 || index >= arr.length)
            throw new Error(`Index out of bounds for ${arrayName}: ${index}`);
      ...
      case "GOSUB": {
        const sortedLines = Array.from(this.program.keys()).sort(
            (a, b) => a - b
        );
      ...
      case "FOR": {
        ...
        const endExpr = rest
            .substring(toIndex + 2, stepIndex > -1 ? stepIndex : rest.length)
            .trim();
        const stepExpr =
            stepIndex > -1 ? rest.substring(stepIndex + 4).trim() : "1";
      ...
      case "NEXT": {
        ...
        if (nextVar && nextVar !== loop.variable)
          throw new Error(
              `NEXT without FOR: expected ${loop.variable} but got ${nextVar}`
          );
        ...
        const finished =
            loop.step > 0 ? currentVal > loop.end : currentVal < loop.end;
      ...
      case "DIM": {
        ...
        this.arrays.set(
            arrayName,
            new Array(size + 1).fill(isStringArray ? "" : 0)
        );
        break;
      }
      case "DATA":
      ...
      case "READ": {
        ...
        for (const varName of varNames) {
          ...
          if (isStringVariable && typeof dataValue !== "string")
            throw new Error("Type mismatch in READ");
          if (!isStringVariable && typeof dataValue !== "number")
            throw new Error("Type mismatch in READ");
      ...
      case "SYS_POKE": {
        ...
        if (args.length !== 4)
          throw new Error("SYS_POKE requires 4 arguments: x, y, char, color");
      ...
      case "SYS_WRITE": {
        ...
        if (sysWriteArgs.length !== 2)
          throw new Error(
              "SYS_WRITE requires 2 arguments: filepath and content"
          );
        ...
        if (!saveResult.success)
          throw new Error(`Failed to write to file: ${saveResult.error}`);
  ...
  async _evaluateExpression(expression) {
    const { CommandExecutor, FileSystemManager, UserManager } = this.dependencies;
      ...
      switch (funcName) {
        ...
        case "RND":
          if (arg > 0) {
            ...
            this.lastRnd =
                (((this.rndSeed * 1103515245 + 12345) / 65536) % 32768) / 32768.0;
          }
          return this.lastRnd;
        case "LEFT$":
        case "RIGHT$":
        case "MID$":
          ...
          const len =
              args.length > 1
                  ? await this._evaluateExpression(args[1])
                  : undefined;
          ...
          if (funcName === "MID$") {
            ...
            const midLen =
                args.length > 2
                    ? await this._evaluateExpression(args[2])
                    : undefined;
        ...
        case "SYS_CMD":
          ...
          const result = await CommandExecutor.processSingleCommand(cmd, {
            isInteractive: false,
          });
    ...
    if (parts.length > 1) {
      ...
      const evaluatedParts = await Promise.all(
          parts.map((p) => this._evaluateSinglePart(p))
      );
